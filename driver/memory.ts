...
import { RoomNotFoundError, RoomNotRunningError, RoomTimeoutError, ServerNotFoundError } from '../common/errors';
...
export function createMemoryDriver(): Driver {
    ...
    const driver: Driver = {
        ...
        _internal: {
            async registerRoom(roomId, roomType, serverId, data, tags): Promise<void> {
            ...
            async unregisterRoom(roomId): Promise<void> {
                deleteClientsForRoom(roomId);
                rooms.delete(roomId);
            },

            async roomReady(roomId, endpoint, roomSecret): Promise<void> {
            ...
            async roomFailure(roomId, _reason): Promise<void> {
                deleteClientsForRoom(roomId);
                rooms.delete(roomId);
            },

            async waitForRoom(roomId, timeoutMs): Promise<RoomInfo> {
            ...
            async getRoomInfo(roomId): Promise<RoomInfo | null> {
                const r = rooms.get(roomId);
            ...
            async addRoomTags(roomId, tags): Promise<void> {
                const r = rooms.get(roomId);
            ...
            async removeRoomTags(roomId, keys): Promise<void> {
                const r = rooms.get(roomId);
                if (!r) throw new RoomNotFoundError(roomId);
                for (const key of keys) delete r.tags[key];
            },

            async getDesiredState(serverId): Promise<DesiredRoom[]> {
            ...
            async reserveClient(roomId, ttl, data, tags): Promise<ClientReservation> {
                const r = rooms.get(roomId);
                if (!r) throw new RoomNotFoundError(roomId);
            ...
            async connectClient(clientId): Promise<void> {
            ...
            async disconnectClient(clientId): Promise<void> {
                clients.delete(clientId);
            },

            async registerServer(options): Promise<void> {
            ...
            async heartbeat(serverId): Promise<void> {
                const s = servers.get(serverId);
                if (s) s.lastHeartbeat = Date.now();
            },

            async unregisterServer(serverId): Promise<void> {
            ...
            async addServerTags(serverId, tags): Promise<void> {
                const s = servers.get(serverId);
            ...
            async removeServerTags(serverId, keys): Promise<void> {
                const s = servers.get(serverId);
                if (!s) throw new ServerNotFoundError(serverId);
            ...
            async getServer(serverId): Promise<ServerInfo | null> {
                const s = servers.get(serverId);
                return s ? serverToInfo(s) : null;
            },

            async subscribeRoomAssignments(serverId, callback): Promise<() => void> {
            ...
            async renewLeader(): Promise<boolean> {
                return true;
            },
